...
(define split3-gal-revision 1)

;GAL oficial URL variable
(define split3-gal-url "http://registry.gimp.org/node/24833")

(define (split3-gal-check)
  (if (defined? 'gal-info)
    (if (< gal-revision split3-gal-revision)
      (begin
	(gimp-message
	  (string-append
	  "GAL revision " (number->string gal-revision) " слишком стар для этого расширанеия.\n"
...
(gimp-image-undo-group-start image)
  ...
  (let* (
	...
	(gal-item-set-name light "Светлый тон")
	(gal-item-set-name dark "Темный тон")
	...
	(if (> aff 0)
	  (begin
	    ...
	    (set! aff-state (car (gal-item-get-visible affect)))

	    ;cheking visibility
	    (if (= aff-state FALSE)
	      (gal-item-set-visible affect TRUE)
	    )
	    (gal-image-insert-layer image affect -1)
	    (gal-item-set-name affect "Контраст")
	    ...
	    (gal-image-lower-item image affect)
	  )
	)
	...
	(if (= srcswitch FALSE)
		...
		(gal-item-set-name reslayer "Результат")
	    )
	)
