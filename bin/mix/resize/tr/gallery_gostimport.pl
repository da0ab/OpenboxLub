...
my $cmd="convert \"$imgfile\" -layers flatten -colorspace RGB -strip -resize \"300x300>\" \"$th_full\"";
		print "$cmd\n";
		system($cmd);
	}
	if(-f $th_full){
	#	$$py.="<h2><a href=\"$realname\"><img src=\"$thumb\" alt=\"$realname\" /></a></h2>\n";
		$$py.="<img src=\"i/t/$realname\" id=\"a$realname\"  onclick=\"enlarge(this);\" longdesc=\"i/$realname\" alt=\"\" title=\"\" />\n";
		$thnum++;
	}
}
